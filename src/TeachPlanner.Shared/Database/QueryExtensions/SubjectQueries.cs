...
public static class SubjectQueries {
    public static async Task<List<CurriculumSubject>> GetCurriculumSubjects(
        ...
        if (includeElaborations)
            return await context.GetSubjectsWithElaborations(new List<SubjectId>(), cancellationToken);

        return await context.GetSubjectsWithoutElaborations(new List<SubjectId>(), cancellationToken);
    }
