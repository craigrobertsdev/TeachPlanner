...
function EditLessonPlanPage() {
  ...
  const { subjectColors } = useThemeContext();
  ...
  function handleSelectContentDescription(event: React.ChangeEvent<HTMLSelectElement>) {
    const selectedContentDescription = contentDescriptions.find((contentDescription) => contentDescription.id === event.target.value);
    if (selectedContentDescription) {
      setContentDescriptions([...contentDescriptions, selectedContentDescription]);
    }
  }
  ...
  return (
    <div className="text-darkGreen">
      ...
      <div className={`flex justify-between items-center mb-3 border border-darkGreen p-2 bg-${subjectColors[lessonPlan.subject.name]}`}>
      ...
      <form className="flex flex-col p-2 justify-center items-center max-w-5xl m-auto">
        <label htmlFor="content-descriptions" className="text-lg font-semibold">
          Content Descriptions
        </label>
        <select name="content-descriptions" className="mb-2 w-full" onSelect={handleSelectContentDescription}>
          {contentDescriptions.map((contentDescription) => (
            <option key={contentDescription.id} value={contentDescription.id}>
        ...
        <textarea className="mb-2 w-full" rows={10} name="planning-notes" value={planningNotes} onChange={handlePlanningNotesChange}></textarea>
